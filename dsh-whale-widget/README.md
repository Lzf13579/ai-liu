> ## 本仓库是什么
>
> 这是 **`dsh-whale-widget` 的合并版（0.3.8-rice）** —— DSH（DeepSeek Harness）Web 界面右下角的
> 余额小鲸鱼挂件，并已把下面两样合并进本体，**只需要装这一个插件**：
>
> | 组成 | 来源 | 许可 |
> |---|---|---|
...
>
> **安装**：见 [`安装说明.md`](安装说明.md)。一句话版 ——
>
> ```
> dsh plugin --profile web add link:<本仓库目录的绝对路径>
> ```
>
> 然后**完整重启 DSH 桌面端**，浏览器 `Ctrl+F5`。
>
> **⚠️ 授权边界（先读这条，避免误以为坏了）**：`art/` 里**只有 `rice.png` 随包发布**（已获鲸鱼作者授权）。
> 碗 / 饭 / 吃饭音效 / 彩蛋图标与整个 `songs/` 属于**第三方游戏素材**，本仓库的 `.gitignore`
> 已刻意把它们排除。因此从本仓库 clone 下来后：**碗和米粒会退回内联 SVG 画的兜底图形、吃饭没有声音**——
> 这是**预期行为**，不是缺陷。想补齐：在插件面板里用「选择图片…」「选择音频…」换成你自己的素材。
> 详见 [`PROVENANCE.md`](PROVENANCE.md)。
>
> 下面这段是**原项目（鲸鱼挂件上游）的 README**，保持原样以便对照。
>
> ---
